...
//    else if ((opponentAboveMe) || (myScoreIsHigher)) {  // chase opponent closer so it cannot take your picture
    else if (!pointsBlockAvailable) {  // chase opponent closer so it cannot take your picture
        stepCloser[0] = (myZRState[0] + otherZRState[0])/2.0f;
        stepCloser[1] = (myZRState[1] + otherZRState[1])/2.0f;
        stepCloser[2] = (myZRState[2] + otherZRState[2])/2.0f;
      if (!opponentAboveMe) stepCloser[2] = -0.5f;
/*        if (!opponentAboveMe) {
            memcpy(stepCloser, otherZRState, sizeof(float)*3);
            stepCloser[2] = -0.45f;
        }
*/
...
//     DEBUG (("In Points Block capturing...\/n"));
...
//    if ((!opponentTooClose) && (myMirrorStatus >= 0) && game.isFacingOther() && (game.getMirrorTimeRemaining()==0) && (!game.posInDark(otherZRState)) && (game.getEnergy() >= (1.0f+MIN_ENERGY))) {  // opponent SPHERE is in view for picture taking
    if ((!opponentTooClose) && game.isFacingOther() && (!game.posInDark(otherZRState)) && (game.getEnergy() >= 1.8f)) {  // opponent SPHERE is in view for picture taking
        picPoints = game.getPicPoints();  // check how many points the picture worths
//      DEBUG(("This picture is worth = %f pts", picPoints));
        if (game.isCameraOn() && (game.getEnergy() >= 1.7f) && (picPoints > 0.0f) && opponentExposed) { // check conditions for picture taking
...
//          DEBUG(("This picture taken is worth = %f pts", picPoints));
    ...
    if ((game.getMemoryFilled() > 1) && (cosineTheta >= ZR_MAX_FACING_ANGLE) && (mathVecMagnitude(&myZRState[9], 3) <= 0.05f) && (game.getEnergy() >= 1.7f)) { //check if we have more than one pictures to upload
        game.uploadPics();  // upload picture if we have pictures
//        DEBUG(("Uploading pics\n"));
    }
//    DEBUG(("Time : , %d", gameTime));
    if ((game.getMemoryFilled() > 0) && ((gameTime > 160 ) || (opponentTooClose))) { //check if we have pictures to upload near end game
...
*/   
    if (gameTime == 180) DEBUG(("<=== Color of ZiRconiuM_Paly_Eagles strategy TE6A ===\n"));
