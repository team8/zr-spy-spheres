...
//conta quanti mirror ha preso l'avversario
    enemyMirror = 0;
    if(info.item.mirror[0] == 1) enemyMirror++;
    if(info.item.mirror[1] == 1) enemyMirror++;
    if(enemyMirror == 0) return false;//se non ne ha preso neanche 1 restituisce false
//------------------------------------------
    
//se l'avversario ha usato il mirror verifica se e' ancora attivo
    if(enemyMirrorUsed > 0)
    {
        if((info.elapsedTime - timeUsedMirror) <= MIRROR_TIME)  return true;//se e' ancora attivo restituisce true
        if((enemyMirror - enemyMirrorUsed) == 0) return false;//se non ha più mirror a disposizione restituisce false
    }
//--------------------------------------------------------------------
...
void update()
{
    ...
    {
        if(id < ENERGY_NUM) info.item.energy[id] = game.hasItem(id);
        else if(id < ENERGY_NUM + SCORE_NUM) info.item.score[id-ENERGY_NUM] = game.hasItem(id);
        else if(id < ENERGY_NUM + SCORE_NUM + MIRROR_NUM) info.item.mirror[id-(ENERGY_NUM+SCORE_NUM)] = game.hasItem(id);
    }
}
//End page INFO
...
bool isFacing(float pos[], char id)
{
    if(id == 0)
    {
        ...
        if(mathVecInner(info.myAtt, relativePos, 3) > MAX_LOOK_ANGLE) return true;
        else return false;
    }
    else
    {
        ...
        mathVecNormalize(relativePos, 3);
        if(mathVecInner(info.enemyAtt, relativePos, 3) > MAX_LOOK_ANGLE) return true;
        else return false;
    }
}
//End page IS_FACING
...
//condizioni rispetto la straregia adottata
    if(needUpload()) return upload();  //esegue l'upload se necessario
    else look(info.enemyPos);          //altrimenti punta l'altro spheres
    
    if(info.myEnergy < 1.5/*2*/)   return -2;         //verifica se abbiamo sufficente energia per fare la foto
//----------------------------------------------------------

//condizioni obbligatorie per il successo della foto
    if(game.getMirrorTimeRemaining() != 0) return -3; //verifica che non stiamo usando il mirror
    if(!game.isCameraOn()) return -4;                 //verifica se la camera è ativa
    if(!isFacing(info.enemyPos, 0)) return -5;        //verifica se stiamo puntando l'avversario
    if(game.posInDark(info.enemyPos)) return -6;      //verifica se l'avversario sitrova in una zona dove si può fotografare
    if(distance(info.myPos, info.enemyPos) >= MIN_PIC_DIST)
        if(enemyUsedMirror(true)) return -7;     //verifica se l'avversario non sta usando il mirror
//----------------------------------------------------------
...
bool needUpload() //verifica se c'e' bisognio dell'upload o meno
{
    if(info.usedMemory < MEMORY_MAX)
    {
        ...
        if(info.usedMemory > 0)
            if(info.elapsedTime > MAX_TIME - 20) return true;
...
//PAGE: CONSTANTS
    ...
    for(int id = 0; id < ITEM_NUM; id++)
    {
        if(id < ENERGY_NUM) game.getItemLoc(ENERGY[id], id);
        else if(id < ENERGY_NUM + SCORE_NUM) game.getItemLoc(SCORE[id-ENERGY_NUM], id);
        else if(id < ENERGY_NUM + SCORE_NUM + MIRROR_NUM) game.getItemLoc(MIRROR[id-(ENERGY_NUM+SCORE_NUM)], id);
    }
//-------------------------------
}
//End page SETUP
...
void strategy()
{
    switch(strategyID)
    {
        ...
        case 1: //prende il mirror indicato dalla fase precedente
            if(info.item.mirror[itemID] != -1) strategyID++;
            move(MIRROR[itemID]);
            break;
        case 2:
            shield();
            itemID = next(2);
            if(itemID == -1) strategyID++;
            else move(MIRROR[itemID]);
            break;
